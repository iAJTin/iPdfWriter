...
namespace iPdfWriter.Code
{
    ...
    /// <summary>
    /// Shows the use of add an enumerable (native render) in a pdf document asynchronously.
    /// </summary>
    internal static class Sample17
    {
        public static async Task GenerateAsync(ILogger logger, YesNo useTestMode = YesNo.No, CancellationToken cancellationToken = default)
        {
            ...
            var saveResult = await result.Result.Action(new SaveToFileAsync { OutputPath = "~/Output/Sample17/Sample-17" }, cancellationToken);
