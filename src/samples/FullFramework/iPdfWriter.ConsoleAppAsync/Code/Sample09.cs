...
namespace iPdfWriter.ConsoleApp.Code
{
    ...
    /// <summary>
    /// Shows the use of compression threshold (>2MB) when merge and zip files (NO zipped) asynchronously.
    /// </summary>
    internal static class Sample09
    {
        public static async Task GenerateAsync(ILogger logger, YesNo useTestMode = YesNo.No, CancellationToken cancellationToken = default)
        {
            ...
            var page1 = new PdfInput
            {
                ...
                Input = "~/Resources/Sample-09/file-sample-1.pdf"
            };
            ...
            page1.Replace(new ReplaceText(
                new WithTextObject
                {
                    ...
                    Offset = PointF.Empty,
                    Style = StylesHelper.Sample09.TextStylesTable["ReportTitle"],
            ...
            {
                AutoUpdateChanges = true,
                Input = "~/Resources/Sample-09/file-sample-2.pdf"
            };
            ...
            {
                AutoUpdateChanges = true,
                Input = "~/Resources/Sample-09/file-sample-3.pdf"
            };
            ...
            {
                AutoUpdateChanges = true,
                Input = "~/Resources/Sample-09/file-Sample-4.pdf"
            };
            ...
            var systemTags = new SystemTagsCollection
            {
                new PageNumberSystemTag
                {
                    UseTestMode = useTestMode,
                    Offset = PointF.Empty,
                    Style = StylesHelper.Sample09.TextStylesTable["PageNumber"],
                    ReplaceOptions = ReplaceTextOptions.FromPositionToRightMargin
                }
            };
            ...
            var globalReplacements = new GlobalReplacementsCollection
            {
                new WithTextObject
                {
                    ...
                    Style = StylesHelper.Sample09.TextStylesTable["Header"],
            ...
            var saveResult = await mergeResult.Result.Action(new SaveToFileAsync { OutputPath = "~/Output/Sample09/Sample-09" }, cancellationToken);
            ...
            logger.Info("     > Path: ~/Output/Sample09/Sample-09.pdf");
            logger.Info($"   > Elapsed time: {ts.Hours:00}:{ts.Minutes:00}:{ts.Seconds:00}.{ts.Milliseconds / 10:00}");
