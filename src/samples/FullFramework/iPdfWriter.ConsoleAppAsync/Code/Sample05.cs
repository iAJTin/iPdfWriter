...
namespace iPdfWriter.ConsoleApp.Code
{
    ...
    /// <summary>
    /// Shows the use of <b>System Tags</b> such as page number of a document and the use of merge of several entries to compose a complete document asynchronously.
    /// </summary>
    internal static class Sample05
    {
        public static async Task GenerateAsync(ILogger logger, YesNo useTestMode = YesNo.No, CancellationToken cancellationToken = default)
        {
            ...
            var saveResult = await mergeResult.Result.Action(new SaveToFileAsync { OutputPath = "~/Output/Sample05/Sample-05" }, cancellationToken);
