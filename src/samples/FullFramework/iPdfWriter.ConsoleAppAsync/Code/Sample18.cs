...
namespace iPdfWriter.ConsoleApp.Code
{
    ...
    /// <summary>
    /// Shows the use of add a datatable (render as html) in a pdf document asynchronously.
    /// </summary>
        ...
        public static async Task GenerateAsync(ILogger logger, YesNo useTestMode = YesNo.No, CancellationToken cancellationToken = default)
        {
            ...
            doc.Replace(new ReplaceText(
                new WithTableObject
                {
                    ...
                    Table = await PdfTable.CreateFromDataTableAsync(
                        data:
                        new List<Person>
                            {
            ...
            var saveResult = await result.Result.Action(new SaveToFileAsync { OutputPath = "~/Output/Sample18/Sample-18" }, cancellationToken);
