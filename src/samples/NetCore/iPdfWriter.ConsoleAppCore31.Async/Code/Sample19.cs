...
namespace iPdfWriter.Code
{
    ...
    /// <summary>
    /// Shows the use of add a datatable (native render) in a pdf document asynchronously.
    /// </summary>
    internal static class Sample19
    {
        public static async Task GenerateAsync(ILogger logger, YesNo useTestMode = YesNo.No, CancellationToken cancellationToken = default)
        {
            ...
            var saveResult = await result.Result.Action(new SaveToFileAsync { OutputPath = "~/Output/Sample19/Sample-19" }, cancellationToken);
