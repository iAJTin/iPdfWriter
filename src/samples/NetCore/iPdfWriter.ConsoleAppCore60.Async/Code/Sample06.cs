...
namespace iPdfWriter.Code
{
    ...
    /// <summary>
    /// Shows the use of test mode asynchronously.
    /// </summary>
    internal static class Sample06
    {
        public static async Task GenerateAsync(ILogger logger, YesNo useTestMode = YesNo.No, CancellationToken cancellationToken = default)
        {
            ...
            var saveResult = await mergeResult.Result.Action(new SaveToFileAsync { OutputPath = "~/Output/Sample06/Sample-06" }, cancellationToken);
